...
export function Step1Welcome({ data, update }: StepProps) {
  return (
    <div>
      <p className="text-[11px] uppercase tracking-[0.22em] text-life font-sans mb-4">
      ...
      <ul className="grid sm:grid-cols-3 gap-4 mb-10">
        {[
          ...
          <li key={item.title} className="border-t border-life/25 pt-4">
      ...
      <label className="flex items-start gap-3 cursor-pointer rounded-lg border border-border-medium bg-ivory p-5">
        <input
          ...
          className="mt-1 w-5 h-5 rounded accent-life flex-shrink-0"
          aria-required="true"
        />
